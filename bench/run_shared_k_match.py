...
def main():
    ...
    # Phase 1: greedy shared selection
    ...
    while True:
        ...
        for i in range(len(X_te)):
            if coverage[i] < K and j_star in inrad_lists[i]:
                coverage[i] += 1
        if int((coverage >= K).sum()) == len(X_te):
            break
    ...
    }).to_csv(out_dir/"preds_shared_k.csv", index=False)

    cols = [f"train_{j}" for j in range(len(X_tr))]
    um = pd.DataFrame(usage_mat, columns=cols); um.insert(0, "target_idx", list(range(len(X_te))))
    um.to_csv(out_dir/"usage_matrix_shared_k.csv", index=False)

    pd.DataFrame(usage_edges, columns=["target_idx","train_idx","rank"]).to_csv(out_dir/"usage_edges_shared_k.csv", index=False)

    summary = dict(setup="RaRF-Shared(k)", tau=TAU, k=K, n_test=int(len(X_te)),
                   mae=float(mae), rmse=float(rmse), r2=float(r2), union=int(union),
                   mean_neighbors_per_target=float(usage_mat.sum(axis=1).mean()))
    Path("summary_shared_k.json").write_text(json.dumps(summary, indent=2))
