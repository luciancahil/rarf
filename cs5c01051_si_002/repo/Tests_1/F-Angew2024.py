# Case study: magnesium reactions (*Angew. Chem. Int. Ed.* **2024**)
...
for i in [0.9]:
    ...
    radius_pred, train_neighbours = RaRFRegressor.RaRFRegressor(radius=i, metric='jaccard').train_parallel(X_train,y_train, include_self='True', distances=train_distances)
    radius_testpred, test_neighbours = RaRFRegressor.RaRFRegressor(radius=i,metric='jaccard').predict_parallel(X_train, y_train, X_test, distances=distances)    
    ...
    plt.tight_layout()
    plt.savefig('../results/F-Angew2024.png',dpi=300)
    plt.clf()
